...
With many devices and cards this utility eases card identification and maintenance, as the microSD card may be difficult to access. The utility also provides an estimate of the cards age from the manufacturing date to facilitate management based on age if desired.  
...
The key advantage of 'sdinfo' being a BASH script is Python, other languages or libraries are not required. If preferred, the script can be copy / pasted avoiding install of <a href="https://en.wikipedia.org/wiki/Git" target="_blank">GIT</a> if preferred (though GIT possibly a better option for version management).  
