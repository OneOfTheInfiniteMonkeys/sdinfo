...
readonly COL_Logo=$(tput setaf 3)                 # Yellow
readonly COL_StdText=$(tput setaf 5)              # Magenta
readonly COL_MnuText=$(tput setaf 5)              # Magenta
readonly COL_WhiteText=$(tput setaf 7)            # Bright White
readonly COL_RedText=$(tput setaf 1)              # Bright Red
readonly COL_Tck=$(tput setaf 2)                  # Bright Green
readonly COL_Crs=$(tput setaf 1)                  # Bright Red
readonly COL_Spinner=$(tput setaf 6)              # Cyan
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -a|--additional)
            ...
            ;;
        -m|--minimal)
            printf "      - Minimal output infomation. i.e. No banners or other messages\n"
            cmd_minimal="y"
            ;;
        -s|--sourcepath)
            ...
            printf "      - Parameter specifying alternate path to search for CID file\n"
            ;;
        -v|--version)
            printf "      %s %s %s ( %s %s ) \n" "$App_Name" "$PROG" "$installer_version" "$installer_date" "$installer_time"
            exit 0
            ;;
        -h|--help|-?)
            printf " Usage: %s [OPTION...] [COMMAND]...\n" "$PROG"
            printf " Options:\n"
            printf "   -a, --additional       Output additional information during installation\n"
            printf "   -m, --minimal          Only minimal ouptut from the script is output.\n"
...
#------------------:
# Globals          :
#                  : $COL_Logo,  ${COL_WhiteText}, ${App_Name}, ${installer_date}
#------------------:
...
cidpath=$(find / -name 'cid' -print 2>/dev/null)
cidinfo=$(cat $cidpath)
printf "\r                                  "
printf "\n"
...
# Decode the Part Number from hex to decimal
d_pnm=""
